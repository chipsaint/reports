...
if password ==st.secrets['report_password']:
    ...
    st.markdown('For the simulation of Energy Arbitrage with charging from WESM, charging cost is computed as: <br/><center>$Total\,Cost = \sum Charge(kW)* Hourly \, WESM \, Rate(PhP/kWh)$</center><br/>Likewise, the total income is computed as:<br/><center>$Total\,Income = \sum Discharge(kW)* Hourly \, WESM \, Rate(PhP/kWh)$</center><br/>The profit is calculated as:<br/><center>$Profit = Income - Cost$</center><br/>',unsafe_allow_html=True)
