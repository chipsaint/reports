...
if password =='etesreport':
    st.title('Echogen DER-VET Results Analysis')
    st.text('July 2021 | Prepared by E. Ramos & D. Santos')
    ...
    st.markdown('R. Wackerly provided the sample data for a 100MW/10h system. The simulated schedule provided by DER-VET is illustrated below.')
    st.plotly_chart(charts.chart_echogen_schedule())
    st.markdown('The ETES was charged during 22:00-6:00 of the next day. The amount of discharged power varied per day. One trend observed was that the power was usually discharged when the WESM price for the hour was above 3.00 Php/kw')
    st.plotly_chart(charts.chart_echogen_selling_price())
    st.markdown('Analysis of the charging & discharging schedule computed by DER-VET showed that there are other factors involved in determining power dispatch. Total Charge and Total Discharge were taken from the data provided. Available Charge is computed as 63% (RTE) of the Total Charge. Battery Balance is the amount of power surplus/deficit for the given cycle.')
    ...
    st.markdown('The model results for 2022 data were simulated accordingly:<ul><li><b>Uncontrained</b> - Stored energy was discharged at the hours with the highest WESM rates.</li><li><b>Constrained</b> - Stored energy was discharged at the hours with the highest WESM rate and exceeded the minimum rate specified.</li>',unsafe_allow_html=True)
    ...
    st.dataframe(charts.table_LT_model().set_index('Model'))
